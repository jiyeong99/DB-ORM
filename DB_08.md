...
for mv in movies:
    movie = Movie()
    movie.director = Director.objects.get(name=mv[0])
    movie.genre = Genre.objects.get(title=mv[1])
    movie.title = mv[2]
    movie.opening_date = mv[3]
    movie.running_time = mv[4]
    movie.screening = mv[5]
    movie.save()
```
...
```python
movies = Movie.objects.order_by('opening_date')
print(movies[0].director.name, movies[0].genre.title, movies[0].title, movies[0].opening_date, movies[0].running_time, movies[0].screening)
```
...
```python
movies = Movie.objects.filter(screening=1).order_by('opening_date')
for movie in movies:
